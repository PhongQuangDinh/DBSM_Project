...
go
CREATE or alter PROCEDURE sp_insertMedicalRecord
...
BEGIN TRAN
	...
	begin
		raiserror(N'Cuộc hẹn không tồn tại', 16, 1)
		rollback tran
		return
	end
	if not exists(select 1 from Person de where de.person_id = @patientID)
	begin
		raiserror(N'Bệnh nhân không tồn tại', 16, 1)
		rollback tran
		return
	end
	if not exists(select 1 from Person de where de.person_id = @patientID and de.person_type = 'PA')
	begin
		raiserror(N'Không phải là bệnh nhân', 16, 1)
		rollback tran
		return
	...
	if not exists(select 1 from Person de where de.person_id = @dentistID and de.person_type = 'DE')
	begin
